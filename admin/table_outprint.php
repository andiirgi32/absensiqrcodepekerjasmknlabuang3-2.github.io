...
<head>
  ...
  <title>Ruang<?= $_SESSION['role'] ?> - Table Out Print</title>
...
<body id="page-top">
        ...
        <li class="nav-item">
          ...
          <div id="collapseTable" class="collapse show" aria-labelledby="headingTable" data-parent="#accordionSidebar">
            <div class="card py-2 collapse-inner rounded">
              ...
              <a class="collapse-item active btn-light" href="table_outprint.php">Table Out Print</a>
      ...
      <div id="content">
        ...
        <div class="container-fluid bg-body-tertiary" id="container-wrapper">
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0">DataTable Out Print</h1>
            <ol class="breadcrumb">
              <li class="breadcrumb-item"><a href="./">Home</a></li>
              <li class="breadcrumb-item" aria-current="page">Tables</li>
              <li class="breadcrumb-item active" aria-current="page">DataTable Out Print</li>
            </ol>
          </div>
            ...
            <div class="col-lg-12">
              <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary">Table Out Print</h6>
                  <?php
                  if ($_SESSION['role'] == "Admin") {
                  ?>
                    <a href="" data-bs-toggle="modal" data-bs-target="#tambahOutPrint" class="btn btn-primary btn-sm"><i class="bi bi-file-earmark-plus-fill"></i></a>
                  <?php
                  }
                  ?>
                </div>
                <div class="table-responsive p-3">
                  <table class="table align-items-center table-flush table-striped table-hover" id="dataTable">
                    <thead class="thead-light">
                      <tr>
                        <th>No</th>
                        <th>Nama Cetak Daftar Absensi Keluar Sebagai</th>
                        <?php
                        if ($_SESSION['role'] == "Admin") {
                        ?>
                          <th>Aksi</th>
                    ...
                    <tbody>
                      <?php
                      $sql = mysqli_query($conn, "SELECT * FROM outprint");
                      $no = 1;
                      while ($data = mysqli_fetch_array($sql)) {
                      ?>
                        <tr>
                          <td><?= $no++ ?></td>
                          <td><?= $data['outprintas'] ?></td>
                          <?php
                          if ($_SESSION['role'] == "Admin") {
                          ?>
                            <td>
                              <a href="" data-bs-toggle="modal" data-bs-target="#editoutprint<?= $data['outprintid'] ?>" class="btn btn-warning mb-1"><i class="bi bi-pencil-square"></i></a>
                              <form action="hapus_outprint.php?outprintid=<?= $data['outprintid'] ?>" method="post" onsubmit="return confirm('Apakah kamu yakin ingin menghapusnya?')" style="display: inline;">
  ...
  <!-- Modal Scrollable -->
  <div class="modal fade" id="tambahOutPrint" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
      <form class="modal-content" action="proses_outprint.php" method="post">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalScrollableTitle">Tambah Nama Cetak File</h5>
          <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <label for="outprint">Nama Cetak Absensi Sebagai</label>
          <select name="outprintas" id="outprint" class="form-control mb-2" required>
            <option value="">Pilihan</option>
            <option value="PEGAWAI NEGERI SIPIL (PNS) DAN PPPK">PEGAWAI NEGERI SIPIL (PNS) DAN PPPK</option>
            <option value="GURU TIDAK TETAP ( GTT )">GURU TIDAK TETAP ( GTT )</option>
            <option value="PEGAWAI TIDAK TETAP ( PTT )">PEGAWAI TIDAK TETAP ( PTT )</option>
            <option value="other">Lainnya</option> <!-- Opsi untuk menampilkan input teks -->
          </select>

          <input type="text" id="outprint_other" class="form-control mb-2" placeholder="Masukkan kelas lainnya" style="display: none;" required>

          <script>
            document.addEventListener('DOMContentLoaded', () => {
              const selectElement = document.getElementById('outprint');
              const inputOther = document.getElementById('outprint_other');
              ...
              inputOther.addEventListener('input', () => {
                ...
                selectElement.options[5] = new Option(newOption.textContent, newOption.value);

                const SelectedCostumOption = selectElement.options[5];
  ...
  <?php
  $kelasArray = [
    "PEGAWAI NEGERI SIPIL (PNS) DAN PPPK", "GURU TIDAK TETAP ( GTT )", "PEGAWAI TIDAK TETAP ( PTT )",
  ];
  ...
  ?>
    <!-- Modal Scrollable -->
    <div class="modal fade" id="editoutprint<?= $data['outprintid'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
        <form class="modal-content" action="update_outprint.php" method="post">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalScrollableTitle">Edit Nama Cetak File</h5>
            <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <input type="text" name="outprintid" value="<?= $data['outprintid'] ?>" hidden required>
            <label for="outprintEdit<?= $index ?>">Cetak Keluar Sebagai:</label>
            <select name="outprintas" id="outprintEdit<?= $index ?>" class="form-control mb-2" required>
              <?php
              // Jika $data['outprintas'] tidak ada di $kelasArray, tambahkan sebagai opsi
              if (!in_array($data['outprintas'], $kelasArray)) : ?>
                <option value="<?= $data['outprintas'] ?>" selected><?= $data['outprintas'] ?></option>
              <?php endif; ?>
              <option value="PEGAWAI NEGERI SIPIL (PNS) DAN PPPK" <?= $data['outprintas'] == "PEGAWAI NEGERI SIPIL (PNS) DAN PPPK" ? "selected" : "" ?>>PEGAWAI NEGERI SIPIL (PNS) DAN PPPK</option>
              <option value="GURU TIDAK TETAP ( GTT )" <?= $data['outprintas'] == "GURU TIDAK TETAP ( GTT )" ? "selected" : "" ?>>GURU TIDAK TETAP ( GTT )</option>
              <option value="PEGAWAI TIDAK TETAP ( PTT )" <?= $data['outprintas'] == "PEGAWAI TIDAK TETAP ( PTT )" ? "selected" : "" ?>>PEGAWAI TIDAK TETAP ( PTT )</option>
              <option value="other">Lainnya</option> <!-- Opsi untuk menampilkan input teks -->
            </select>
            <input type="text" id="outprintEdit_other<?= $index ?>" class="form-control mb-2" placeholder="Masukkan kelas lainnya" value="<?= $data['outprintas'] ?>" style="display: none;" required>
            <script>
              document.addEventListener('DOMContentLoaded', () => {
                const selectElementEdit = document.getElementById('outprintEdit<?= $index ?>');
                const inputOtherEdit = document.getElementById('outprintEdit_other<?= $index ?>');
